...
def transmit_demo():
    ...
    # ------------------------------------------------------------------
    # Step 1: Send Diagnostic Session Control (10 02) - Program Session
    # ------------------------------------------------------------------
    app.write_log(LOG_LVL_INFO, "[Step 1] Sending 10 03 (extend Session)")
    send_can_frame([0x02, 0x10, 0x03, 0x00, 0x00, 0x00, 0x00, 0x00])
    time.sleep(0.1)
    ...
    # ------------------------------------------------------------------
    app.write_log(LOG_LVL_INFO, "[Step 1] Sending 10 02 (Program Session)")
    send_can_frame([0x02, 0x10, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00])
    time.sleep(0.1)
    # ------------------------------------------------------------------
    # Step 4: Build and Send 2E F1 5A Multi-Frame
    # ------------------------------------------------------------------
    app.write_log(LOG_LVL_INFO, "[Step 4] Building 2E F1 5A ISO-TP Multi-Frame")
    frames = build_isotp_multiframe(FINGERPRINT_DATA)
    app.write_log(LOG_LVL_INFO, f"Total frames to send: {len(frames)}")

    # Send FF
    app.write_log(LOG_LVL_INFO, f"Sending FF: { [hex(b) for b in frames[0]] }")
    send_can_frame(frames[0])

    # Wait for Flow Control from ECU (30 XX XX ...)
    # In real test, parse FC to get STmin and BlockSize.
    # Here we use a fixed delay.
    app.write_log(LOG_LVL_INFO, "Waiting 100ms for ECU FlowControl...")
    time.sleep(0.1)

    # Send CFs with STmin = 10ms
    stmin_ms = 0.01
    for i in range(1, len(frames)):
        time.sleep(stmin_ms)
        app.write_log(LOG_LVL_INFO, f"Sending CF{i}: { [hex(b) for b in frames[i]] }")
        send_can_frame(frames[i])

    app.write_log(LOG_LVL_INFO, "All frames sent. Waiting for ECU response...")
    time.sleep(0.5)
    app.write_log(LOG_LVL_INFO, "Test completed.")
