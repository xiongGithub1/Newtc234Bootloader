...
/**
 * @brief ????????? App ???锟斤拷??Sector 6??
 * @note ??? Flash ?????????????锟斤拷???锟斤拷?? 0xA55A??
 */
...
// 锟斤拷锟斤拷值: 锟斤拷8位 = canFlash (1=锟斤拷, 0=锟斤拷锟斤拷), 锟斤拷8位 = targetBank ('A' 锟斤拷 'B')
		...
		case 0x01:
			{
				...
				switch (routineIdentifier)
				{
					...
					case 0xFFFD:
						...
						m_pstPDUMsg->aDataBuf[4] = (uint8) (routineResult >> 8);   /* canFlash: 1=锟斤拷, 0=锟斤拷锟斤拷 */

						m_pstPDUMsg->aDataBuf[5] = (uint8) (routineResult & 0xFF); /* targetBank: 'A' 锟斤拷 'B' */
						m_pstPDUMsg->xDataLen = 6;
						break;
						// #endif	
					// erase flash
